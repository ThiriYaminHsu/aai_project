"""Chapter-4 comparison barplot: mean % TOTAL gate reduction by final agent
...
circuits). Agents only (no baselines). Writes figures/comparison_barplot_gates_v3.png into the LaTeX tree.
...
def main():
    ...
    df["gate_red"] = 100 * (df["initial_gates"] - df["final_gates"]) / df["initial_gates"]
    ...
    for i, (m, lab, col) in enumerate(zip(methods, labels, colors)):
        sub = df[df["method"] == m]
        means = [sub[sub["family"] == f]["gate_red"].mean() for f in fams]
        stds = [sub[sub["family"] == f]["gate_red"].std() for f in fams]
        ax.bar(x + (i - 1) * w, means, w, yerr=stds, capsize=2, label=lab, color=col)
    ax.axhline(0, color="black", lw=0.8)
    ax.set_ylabel("% reduction in total gate count")
    ax.set_xticks(x)
    ax.set_xticklabels(fam_labels)
    ax.legend(fontsize=9, loc="upper right")
    ax.set_title("Total gate reduction by final agent and evaluation family (best-of-10)")
    fig.tight_layout()
    out = LATEX_FIGS / "comparison_barplot_gates_v3.png"
